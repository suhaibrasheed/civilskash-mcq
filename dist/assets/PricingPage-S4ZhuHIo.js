import{b as re,aR as _e,d as Se,ah as Ce,u as Ee,aS as Ie,r as f,j as e,bc as Pe,k as w,A as ze,aT as ee,ar as Re,aU as B,aB as Te,aq as Oe,bl as ue,q as Me,a$ as xe,aZ as pe,l as Ae,bk as Fe,bm as De,aV as ae,P as te,X as Le,ap as qe,al as Ue,b0 as $e,O as We,s as z,bn as He}from"./index-HiPAAseh.js";import{S as Be}from"./ScratchCardSection-C3zpzHFp.js";/**
...
Join me here --> ${t}`;if(navigator.share)try{await navigator.share({title:"MCQ Kash",text:r,url:t}),i("Referral shared successfully! 🚀","success")}catch(s){s.name!=="AbortError"&&(console.error("Web Share failed:",s),ie(r))}else ie(r)},ie=t=>{navigator.clipboard.writeText(t),i("Share text copied to clipboard! 📋","success")},oe=()=>new Promise(t=>{if(window.Razorpay)return t(!0);const r=document.createElement("script");r.src="https://checkout.razorpay.com/v1/checkout.js",r.onload=()=>t(!0),r.onerror=()=>t(!1),document.body.appendChild(r)}),ve=async t=>{if(!c){i("Sign In to upgrade your account!","warning"),h("/signin");return}if((a==null?void 0:a.user_tier)==="Pro"&&(a==null?void 0:a.pro_tier)===t.id){i("You are already on this plan!","info");return}N(t.id);const r=setTimeout(()=>{N(s=>s===t.id?null:s)},8e3);try{if(!await oe()){clearTimeout(r),i("Failed to load Razorpay.","error"),N(null);return}const{data:{session:x}}=await z.auth.getSession();if(!(x!=null&&x.access_token)){clearTimeout(r),i("Session expired. Sign in again.","warning"),h("/profile"),N(null);return}const n=x.access_token,d=Number((a==null?void 0:a.premium_discount_earned)??S()*25);try{await z.from("profiles").update({premium_discount_earned:d}).eq("id",c.id)}catch(u){console.warn("Pre-checkout discount sync notice:",u)}let p=null,m=null,l="INR",j="rzp_live_SxuAK5B53kL3qS";const{baseMrp:k,couponDiscountPercent:we,couponPrice:Xe,effectiveWalletDiscount:Ne,finalPrice:ke}=fe(t,o,d),ce=ke*100;try{const u=await fetch(`${me}/create-order`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${n}`},body:JSON.stringify({planId:t.id,discount:Ne,couponCode:o==null?void 0:o.code,couponDiscount:we})});if(u.ok){const b=await u.json();b.orderId&&(p=b.orderId),b.amount&&(m=Number(b.amount)),l=b.currency||"INR",b.keyId&&(j=b.keyId)}else console.warn("Remote Edge Function create-order returned non-200, using client amount calculation...")}catch(u){console.warn("Remote Edge Function create-order network error, using client amount calculation...",u)}const de={key:j,amount:ce,currency:l||"INR",name:"MCQ Kash",description:`${t.name} Pro Upgrade`,prefill:{email:c.email},theme:{color:t.featured?"#a855f7":"#f59e0b"},modal:{ondismiss:()=>{clearTimeout(r),N(null),i("Cancelled.","info")}},handler:async u=>{clearTimeout(r);try{let b=!1,_=null,A=null;try{_=await fetch(`${me}/verify-payment`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${n}`},body:JSON.stringify({razorpay_order_id:u.razorpay_order_id||p||"test_order_1day",razorpay_payment_id:u.razorpay_payment_id,razorpay_signature:u.razorpay_signature||"test_sig",planId:t.id})})}catch(v){A=v,console.warn("Network fetch to verify-payment failed:",v.message)}if(_&&_.ok){const{success:v}=await _.json();b=v}else{const v=_?await _.json().catch(()=>({})):{};if(console.warn("Backend verify-payment non-200 or network error, attempting client fallback profile sync...",v,A),u.razorpay_payment_id){const F=new Date,W=t.days||(t.id==="ONE_DAY"||t.id==="ONE_HOUR"?1:30);F.setTime(F.getTime()+Math.round(W*24*60*60*1e3));const P=F.toISOString();localStorage.setItem(`mcqkash_pro_override_${c.id}`,JSON.stringify({is_pro:!0,pro_tier:t.id,pro_expiration:P,pro_expires_at:P}));const Z=t.id==="ONE_DAY"||t.id==="ONE_HOUR"?"ONE_DAY":t.id,{error:H}=await z.from("profiles").update({is_pro:!0,pro_tier:Z,pro_expiration:P,pro_expires_at:P}).eq("id",c.id);H&&console.warn("Client DB update notice (handled by local override):",H.message),b=!0}else throw new Error(v.error||(A==null?void 0:A.message)||"Payment verification failed.")}if(b){if(d>0&&(c!=null&&c.id)){const P=Math.min(d,Math.max(0,t.price-(typeof t.floorPrice=="number"?t.floorPrice:9))),Z=Math.max(0,d-P);try{await z.from("profiles").update({premium_discount_earned:Z}).eq("id",c.id)}catch(H){console.warn("Wallet balance deduction notice on Pro upgrade:",H)}}const v=new Date,F=t.days||30;v.setTime(v.getTime()+Math.round(F*24*60*60*1e3));const W=v.toISOString();localStorage.setItem(`mcqkash_pro_override_${c.id}`,JSON.stringify({is_pro:!0,pro_tier:t.id,pro_expiration:W,pro_expires_at:W})),localStorage.removeItem(`mcqkash_profile_cache_${c.id}`),localStorage.removeItem(`mcqkash_ranks_cache_${c.id}`),localStorage.removeItem("mcqkash_lb_cache_coins"),localStorage.removeItem("mcqkash_lb_cache_streaks"),He({particleCount:180,spread:100,origin:{y:.5},colors:["#fbbf24","#a855f7","#6366f1","#10b981","#f43f5e"]}),i("Welcome to Pro! ★","success"),await C(!0),window.dispatchEvent(new Event("sync-profile-stats")),setTimeout(()=>h("/profile"),1200)}}catch(b){i(b.message||"Verification failed.","error")}finally{N(null)}}};p&&m===ce&&(de.order_id=p);try{const u=new window.Razorpay(de);u.on("payment.failed",function(b){var _;clearTimeout(r),N(null),i(((_=b.error)==null?void 0:_.description)||"Payment failed.","error")}),u.open()}catch{clearTimeout(r),N(null),i("Could not open Razorpay checkout popup. Please try again.","error")}}catch(s){clearTimeout(r),i(s.message||"Checkout failed.","error"),N(null)}},le=(a==null?void 0:a.user_tier)==="Pro";return e.jsxs(e.Fragment,{children:[e.jsx("style",{children:`
