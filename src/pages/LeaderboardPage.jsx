...
import { Award, Sparkles, MessageSquare, Send, Zap, ChevronUp, Lock, AlertTriangle, Coins, Flame } from 'lucide-react';
    ...
    try {
      ...
      if (type === 'shoutout') {
        try {
          ...
          finalRows = (dbRows || []).filter(p => p.pro_expiration && new Date(p.pro_expiration) > new Date());
        } catch (shoutoutError) {
          console.warn('get_shoutout_feed RPC failed, falling back to profiles query:', shoutoutError);
          try {
            const { data: dbRows, error } = await supabase
              .from('profiles')
              .select('id, full_name, avatar_id, pro_expiration, status_message, last_status_update_at')
  ...
  return (
    <div className="min-h-screen bg-theme-bg pb-32 font-sans">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        {/* Global Shoutbox — glassmorphic (Only shown when Shoutout Feed tab is active) */}
        ...
        <section className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-1 sm:px-2">
            ...
            {/* TRIPLE-CAPSULE CONTROL TOGGLE - Mobile Responsive Grid */}
            <div className="w-full sm:w-auto grid grid-cols-3 sm:flex bg-theme-surface border border-theme-border p-1 rounded-2xl shadow-sm text-center">
              <button
                onClick={() => setLeaderboardType('coins')}
                className={`px-2 sm:px-4 py-1.5 sm:py-1 rounded-xl sm:rounded-lg text-[10px] sm:text-[9px] font-black uppercase tracking-wider transition-all duration-300 ${
                  leaderboardType === 'coins' ? 'bg-theme-primary text-white shadow-sm' : 'text-theme-muted hover:text-theme-text'
                }`}
              >
                KashCoins
              </button>
              <button
                ...
                className={`px-2 sm:px-4 py-1.5 sm:py-1 rounded-xl sm:rounded-lg text-[10px] sm:text-[9px] font-black uppercase tracking-wider transition-all duration-300 ${
                  leaderboardType === 'streaks' ? 'bg-theme-primary text-white shadow-sm' : 'text-theme-muted hover:text-theme-text'
                }`}
              >
                Daily Streaks
              </button>
              <button
                ...
                className={`px-2 sm:px-4 py-1.5 sm:py-1 rounded-xl sm:rounded-lg text-[10px] sm:text-[9px] font-black uppercase tracking-wider transition-all duration-300 ${
                  leaderboardType === 'shoutout' ? 'bg-theme-primary text-white shadow-sm' : 'text-theme-muted hover:text-theme-text'
                }`}
              >
                Shoutout Feed
              </button>
            ...
            {leaderboardData.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-5">
                {leaderboardData.map((profile) => {
                  ...
                  const isPro = profile.pro_expiration ? new Date(profile.pro_expiration) > new Date() : !!profile.is_pro;
                      ...
                      <div className="relative z-10 flex items-center justify-between pt-3">
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className={`w-7 h-7 rounded-full shrink-0 relative ${
                            isPro 
                              ? 'ring-2 ring-amber-500 dark:ring-amber-400 p-[1px] shadow-[0_0_8px_rgba(245,158,11,0.4)] bg-gradient-to-tr from-amber-400 to-amber-600' 
                              : 'border border-theme-border'
                          }`}>
                            <Avatar id={profile.avatar_id || 1} className="w-full h-full rounded-full bg-theme-bg" />
                          </div>
                          <div className="min-w-0">
                            <span className={`font-black text-[11px] tracking-tight truncate flex items-center gap-1 leading-none ${
                              isUser ? 'text-theme-primary' : 'text-theme-text'
                            }`}>
                              {profile.full_name || 'Aspirant'}
                              {isPro && (
                                <Sparkles size={9} className="text-amber-500 shrink-0 fill-amber-500 animate-pulse" />
                              )}
                            </span>
                          </div>
                        </div>
                ...
                {leaderboardData.map((profile, i) => {
                  ...
                  return (
                    <React.Fragment key={profile.id}>
                      ...
                      <div 
                        className={`p-2.5 sm:p-4 flex items-center justify-between gap-2 sm:gap-4 transition-all duration-300 ${
                          isUser 
                      ...
                      >
                        <div className="flex items-center gap-2 sm:gap-3.5 min-w-0 flex-1">
                          {leaderboardType !== 'shoutout' && (
                            <div className={`w-6 h-6 sm:w-7 sm:h-7 rounded-xl flex items-center justify-center font-[900] text-[11px] sm:text-xs shrink-0 transition-all ${
                              rank === 1 ? 'bg-gradient-to-br from-amber-300 via-amber-400 to-amber-500 text-amber-950 shadow-md shadow-amber-500/25 ring-1 ring-amber-300/50' :
                              ...
                              isPro
                                ? 'bg-amber-500/20 border border-amber-500/40 text-theme-text font-black shadow-xs ring-1 ring-amber-500/20'
                                : 'bg-theme-bg/85 border border-theme-border text-theme-text font-black'
                            }`}>
                              {rank}
                            </div>
                          )}
                          ...
                          <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full shrink-0 relative transition-all ${
                            isPro 
                              ? 'ring-2 ring-amber-500 dark:ring-amber-400 p-[1.5px] sm:p-[2px] shadow-[0_0_10px_rgba(245,158,11,0.35)] bg-gradient-to-tr from-amber-400 to-amber-600' 
                              : 'p-[1px] border border-theme-border/60'
                          }`}>
                            <Avatar id={profile.avatar_id || 1} className="w-full h-full rounded-full bg-theme-bg" />
                          </div>
                          ...
                          <div className="min-w-0 flex-1 flex flex-col justify-center">
                            <div className="flex items-center gap-1.5 min-w-0">
                              <span className={`font-[900] text-xs sm:text-sm tracking-tight truncate ${
                                isUser 
                                  ? 'text-theme-primary font-black' 
                                  : isPro
                                    ? theme === 'sepia'
                                      ? 'text-[#2d1604] font-[900]'
                                      : theme === 'light'
                                        ? 'text-[#0a1538] font-[900]'
                                        : 'bg-gradient-to-r from-amber-100 via-amber-200 to-yellow-100 bg-clip-text text-transparent font-black drop-shadow-sm'
                              ...
                              {isPro && (
                                <span className={`px-2 py-0.5 rounded-full text-[8px] sm:text-[9px] font-black tracking-wider uppercase select-none shrink-0 flex items-center gap-0.5 shadow-xs ${
                                  theme === 'sepia'
                                    ? 'bg-gradient-to-r from-amber-800 via-amber-700 to-amber-900 text-amber-50 border border-amber-600/50'
                                    : theme === 'light'
                                      ? 'bg-gradient-to-r from-amber-600 via-amber-500 to-amber-600 text-white border border-amber-400/50'
                                      : 'bg-gradient-to-r from-amber-400 via-amber-500 to-amber-400 text-slate-950 border border-amber-300/60 shadow-[0_0_8px_rgba(251,191,36,0.3)]'
                                }`}>
                                  <Sparkles size={7} className={theme === 'sepia' || theme === 'light' ? 'text-amber-50 shrink-0 fill-amber-50' : 'text-slate-950 shrink-0 fill-slate-950 animate-pulse'} />
                                  PRO
                                </span>
                              )}
                            </div>
                            ...
                            {profile.status_message && isPro ? (
                              <span className={`text-[10px] sm:text-[11px] font-extrabold tracking-tight px-2 py-0.5 sm:px-3 sm:py-1 rounded-lg sm:rounded-xl w-fit mt-1 shadow-2xs border truncate max-w-[130px] xs:max-w-[180px] sm:max-w-md ${
                                theme === 'sepia'
                                  ? 'bg-amber-800/10 border-amber-700/30 text-[#261708]'
                                  : theme === 'light'
                                    ? 'bg-amber-500/10 border-amber-500/30 text-[#0b1639]'
                        ...
                        <div className="text-right shrink-0">
                          {leaderboardType === 'shoutout' ? (
                            <>
                              <div className="text-[10px] sm:text-xs font-bold text-theme-muted tracking-tight">
                                {profile.last_status_update_at 
                                  ? new Date(profile.last_status_update_at).toLocaleDateString([], { month: 'short', day: 'numeric' })
                                  : 'Recent'
                                }
                            ...
                            <div className={`flex items-center justify-end gap-1.5 sm:gap-2 shrink-0 px-2 py-0.5 sm:px-3 sm:py-1 rounded-full border transition-all ${
                              isPro
                                ? 'bg-amber-500/10 border-amber-500/35 text-theme-text font-black shadow-xs ring-1 ring-amber-500/20'
                                : 'bg-theme-bg/90 border-theme-border/60 text-theme-text font-bold shadow-2xs'
                            }`}>
                              <span className="text-xs sm:text-base tracking-tight font-extrabold">
                                {formatKMetric(leaderboardType === 'coins' ? profile.total_coins : profile.streak_days)}
                              </span>
                              {leaderboardType === 'coins' ? (
                                <KashCoinIcon className="w-4 h-4 sm:w-5 sm:h-5 shrink-0 inline-block" />
                              ) : (
