...
async function syncData() {
  try {
    ...
    // 3. Fetch and compile Weekly Global Tests
    console.log('Fetching weekly tests from Supabase...');
    const { data: weeklyTests, error: weeklyTestsError } = await supabase
      .from('weekly_tests')
      .select('*')
      .order('window_start', { ascending: false });

    const WEEKLY_DIR = path.join(TARGET_DIR, 'weekly_tests');
    if (!fs.existsSync(WEEKLY_DIR)) {
      fs.mkdirSync(WEEKLY_DIR, { recursive: true });
    }

    let weeklyChanged = false;
    if (!weeklyTestsError && weeklyTests && weeklyTests.length > 0) {
      const weeklyRegistryEntries = [];
      const weeklyCatalog = [];

      for (const test of weeklyTests) {
        const testFile = `${test.id}.json`;
        const testFilePath = path.join(WEEKLY_DIR, testFile);

        let existingTest = null;
        if (fs.existsSync(testFilePath)) {
          try {
            existingTest = JSON.parse(fs.readFileSync(testFilePath, 'utf-8'));
          } catch (e) {}
        }

        if (JSON.stringify(test) !== JSON.stringify(existingTest)) {
          fs.writeFileSync(testFilePath, JSON.stringify(test, null, 2), 'utf-8');
          console.log(`Updated weekly test file: ${testFile}`);
          weeklyChanged = true;
        }

        // Add to catalog (omit questions_data to keep catalog ultra small)
        const { questions_data, ...metadata } = test;
        weeklyCatalog.push(metadata);

        weeklyRegistryEntries.push(`  "${test.id}": () => import('./weekly_tests/${testFile}')`);
      }

      // Write catalog
      const catalogPath = path.join(TARGET_DIR, 'weekly_catalog.json');
      fs.writeFileSync(catalogPath, JSON.stringify(weeklyCatalog, null, 2), 'utf-8');

      // Write dynamic registry
      const weeklyRegistryPath = path.join(TARGET_DIR, 'weekly_registry.js');
      const weeklyRegistryContent = `// Automatically generated by scripts/sync_data.js. Do not edit manually.
export const WEEKLY_TEST_LOADERS = {
${weeklyRegistryEntries.join(',\n')}
};
`;
      fs.writeFileSync(weeklyRegistryPath, weeklyRegistryContent, 'utf-8');
    }

    // If no questions updated and no exams updated and no weekly tests updated, exit cleanly
    if (!hasQuestionUpdates && !examsChanged && !weeklyChanged) {
      console.log('No new updates found. Exiting cleanly.');
      return;
    }
